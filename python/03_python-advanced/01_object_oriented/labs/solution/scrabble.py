...
class ScrabbleDictionary:
    ...
    def __init__(self, rack_letters: list):
        ...
        if ScrabbleDictionary.__instance is not None:
            raise Exception("This class is a singleton! Use the static get_instance() method")
        else:
            ...
            with open('fr_dictionary.csv') as fr_dict:
                for word in fr_dict.readlines():
                    if len(word) <= 7:
                        sanitized_word = unidecode(word.replace('\n', '').replace('.', '')).upper()
                        ...
                        missing_letters = [l for l in missing_letters_counter.elements()]
...
class RackOfLetters(GACitizenBase):
            ...
            if random() < mutation_rate:
                ...
                if swap_with_idx != i:
                    if isinstance(self._dna[i], int):
                        ...
                        buffer = self._dna[i]
                        print(f'\t\tmuting {before}: swapping letters {self._dna[i]} at position {i} and {self._dna[swap_with_idx]} at position {swap_with_idx}')
    ...
    def reproduce_with(self,
                       other_parent: 'RackOfLetters',
                       crossover_rate: float) -> tuple['RackOfLetters', 'RackOfLetters']:
    ...
    def __str__(self):
        ...
        :return:
        """
        dna = ''.join(str(letter) for letter in self._dna if not isinstance(letter, int))
...
class ScrabbleChallenge(GAChallengeBase):
    def __init__(self, initial_rack: str = None, population_count: int = 1000):
        ...
        self._pop = [RackOfLetters(genome=[l for l in initial_rack]) for _ in range(population_count)]
