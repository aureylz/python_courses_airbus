...
class OneMaxCitizen(GACitizenBase):
    ...
    def reproduce_with(
        self, other_parent: "OneMaxCitizen", crossover_rate: float
    ) -> tuple["OneMaxCitizen", "OneMaxCitizen"]:
        child_1 = OneMaxCitizen(genome=self._dna.copy(), genome_size=self._genome_size)
        child_2 = OneMaxCitizen(
            genome=other_parent._dna.copy(), genome_size=self._genome_size
        )
        if random() < crossover_rate:
            pt = randint(1, len(self._dna) - 2)
            child_1 = OneMaxCitizen(
                genome=self._dna[:pt] + other_parent._dna[pt:],
                genome_size=self._genome_size,
            )
            child_2 = OneMaxCitizen(
                genome=other_parent._dna[:pt] + self._dna[pt:],
                genome_size=self._genome_size,
            )
            print(
...
class OneMaxChallenge(GAChallengeBase):
    def __init__(self, population_count: int, genome_size: int):
        ...
        self._pop = [
            OneMaxCitizen(genome=None, genome_size=genome_size)
