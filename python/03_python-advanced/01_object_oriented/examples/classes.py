...
class SimpleAircraft:
    ...
    def convert(self, to_cargo: bool = None) -> str:
        ...
        if self.is_convertible:
            ...
            raise ValueError(
                f"Sorry, {self.model_name} is not convertible from / to cargo"
            )
...
class Helicopter(AircraftBase):
    ...
    def __init__(
        self, model_name: str, registration_id: str, rotor_power: int, stator_power: int
    ):
    ...
    def refuel(self) -> int:
        ...
        """
        print(
            f"refueling (adapted for helicopter) {type(self).__name__} type {self._model_name} registered under {self._registration_id}"
        )
        return 177
...
class AirplaneBase(AircraftBase, ABC):
    ...
    def refuel(self) -> int:
        ...
        """
        print(
            f"refueling (overridden for aiplanes) {type(self).__name__} type {self._model_name} registered under {self._registration_id}"
        )
        return 1023
...
class Glider(AirplaneBase):
    ...
    def init(self, model_name: str, registration_id: str, wing_size: int):
        super().__init__(
            model_name=model_name, registration_id=registration_id, wing_size=wing_size
        )
...
def main3():
    my_helicopter = Helicopter(
        model_name="H175", registration_id="1234A", rotor_power=175, stator_power=1234
    )
    my_jet = Jet(
        model_name="A220", registration_id="3456H", wing_size=45, engine_brand="rolls"
    )
    my_turbo_prop = TurboProp(
        model_name="A400M", registration_id="9876Q", wing_size=65, propeller_size=254
    )
