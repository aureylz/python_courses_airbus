...
```python
my_a320 = Aircraft('A320')
another_plane = Aircraft('A400', True)
my_cargo = Aircraft('C295', is_military=True, is_cargo=True)
# in this case, we're happy with 'is_military' having the default value of False, no need to specify it
a350 = Aircraft('A350F', is_cargo=True)
```
...
In the below example, we'll append a ```convert()``` method that'll change the cabin layout of our ```Aircraft```.
...
- notice the use of the ```_``` (undesrcore) prefix, this is the Python convention to tell this is a private instance / class variable:
  - Technically it's like the ```protected``` keyword in C++ because child classes can also access it
  - But Python linter will warn you if you try to access a protected variable from outside your class
