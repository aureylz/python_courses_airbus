...
print(f"""4.1.1 - Use one of the datetime module functions to displays the current time in your zone: 
    {datetime.now().time()}""")


def what_time_is_it():
    print(f"""4.1.2 - Wrap the above code into a new function to display the current time in your zone: 
    {datetime.now().time()}""")
...
def what_zulu_time_is_it():
    print(f"""4.1.3 - Modify the earlier created function to display the current time in UTC:
...
def what_zulu_time_is_it_fancy(display_format: str):
    ...
    print(f"""4.1.4 - Modify again the function to accept a time formatter to customize the time display format:
...
def what_time_is_it_smart(display_format: str = '%H:%M:%S', in_zulu_time: bool = False):
    ...
    print(f"""4.1.5 - Now make the parameters optional, if not provided, display time in your current zone:
    {current_date} in {'UTC' if in_zulu_time else 'CET' }""")


def what_time_is_it_smart_39(display_format: str = '%H:%M:%S', time_zone: str = 'CET'):
    print(f"""4.1.5 - Python >= 3.9:
    {datetime.now(ZoneInfo(time_zone)).time().strftime(display_format)} in {time_zone}""")
...
def simple_clock(duration: int = 3):
    print(f"""4.2.1 - Create a function that displays the current time during 1 mn and refresh every second:""")
...
def simple_clock_even_odd(duration: int = 6):
    print(f"""4.2.2 - Modify the above function so that it displays even seconds differently""")
    for sec in range(duration):
...
def simple_timer(duration: int = 12):
    print("""4.2.3 - Create a countdown time that displays a * each second, but starts displaying the 10 remaining seconds""")
